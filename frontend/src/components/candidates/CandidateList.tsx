        ...
        <div className="flex flex-col space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <Button 
              ...
              asChild
            >
              <a href="/candidates/new" className="flex items-center gap-2">
          ...
          <Card className="backdrop-blur-md bg-background/40 border border-indigo-500/20 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
            ...
            <CardHeader className="relative z-10 pb-2">
              <CardTitle className="text-xl font-semibold text-indigo-950 dark:text-indigo-100">Search Candidates</CardTitle>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <Card key={i} className="backdrop-blur-md bg-background/40 border border-indigo-500/20 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-purple-500/5 pointer-events-none" />
                  <CardHeader className="relative z-10 pb-2">
              ...
              {data?.candidates && data.candidates.length > 0 ? (
                  ...
                  {data.candidates.map((candidate) => (
                    ...
                    >
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            ...
                            {candidate.skills.length > 3 && (
                              <Badge 
                                className="px-2 py-0.5 backdrop-blur-sm bg-background/50 border-indigo-500/30 text-indigo-600 dark:text-indigo-300 text-xs font-medium"
                              >
                      ...
                      <CardFooter className="relative z-10 flex justify-between items-center">
                        <Badge 
                          className={`px-2.5 py-0.5 backdrop-blur-sm ${getStatusColors(candidate.status)}`}
                        >
                ...
                <Card className="backdrop-blur-md bg-background/40 border border-indigo-500/20 shadow-xl overflow-hidden text-center py-12">
                  ...
                  <CardContent className="relative z-10">
                    ...
                    <h3 className="text-xl font-semibold mb-2 text-indigo-950 dark:text-indigo-100">No candidates found</h3>
                    <p className="text-indigo-700/70 dark:text-indigo-300/70 max-w-md mx-auto">
                      Try adjusting your search or filter criteria, or add a new candidate to get started.
                    </p>
                  </CardContent>
                </Card>
              )}
                    ...
                    <PaginationContent>
                      <PaginationItem>
                        <PaginationPrevious
                          ...
                          className={`${page <= 1 ? 'pointer-events-none opacity-50' : ''} backdrop-blur-sm bg-background/50 border-indigo-500/30 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50/20 dark:hover:bg-indigo-950/20 transition-all duration-300`}
                        />
                      </PaginationItem>
                      {Array.from({ length: data.pagination.totalPages }, (_, i) => i + 1).map((p) => (
                        <PaginationItem key={p}>
                          <PaginationLink
                            ...
                            isActive={page === p}
                            className={`${page === p ? 'bg-indigo-500 text-white' : 'backdrop-blur-sm bg-background/50 border-indigo-500/30 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50/20 dark:hover:bg-indigo-950/20'} transition-all duration-300`}
                          >
                            {p}
                          </PaginationLink>
                        </PaginationItem>
                      ...
                      <PaginationItem>
                        <PaginationNext
                          ...
                          className={`${page >= data.pagination.totalPages ? 'pointer-events-none opacity-50' : ''} backdrop-blur-sm bg-background/50 border-indigo-500/30 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50/20 dark:hover:bg-indigo-950/20 transition-all duration-300`}
                        />
                      </PaginationItem>
                    </PaginationContent>
                  </Pagination>
