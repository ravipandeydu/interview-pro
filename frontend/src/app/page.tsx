...
import { CheckCircle, Code, Users, VideoIcon, ClipboardCheck, Brain, Shield, Zap, ArrowRight } from 'lucide-react'
...
export default function Home() {
  ...
  // Feature card colors
  const featureColors = [
    { bg: 'bg-purple-500/10', border: 'border-purple-500/30', icon: 'text-purple-500' },
    { bg: 'bg-blue-500/10', border: 'border-blue-500/30', icon: 'text-blue-500' },
    { bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', icon: 'text-emerald-500' },
    { bg: 'bg-amber-500/10', border: 'border-amber-500/30', icon: 'text-amber-500' },
    { bg: 'bg-rose-500/10', border: 'border-rose-500/30', icon: 'text-rose-500' },
    { bg: 'bg-cyan-500/10', border: 'border-cyan-500/30', icon: 'text-cyan-500' },
  ]
      ...
      <div className="container mx-auto px-4 py-16 relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {[
              ...
              <Badge 
                key={index} 
                variant="secondary" 
                className={`px-3 py-1 backdrop-blur-sm border ${badgeColors[index % badgeColors.length]}`}
              >
                {label}
              </Badge>
            ))}
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            {enableAuth ? (
              <Button size="lg" className="backdrop-blur-sm bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg shadow-indigo-500/30 border-0 transition-all duration-300 hover:scale-105" asChild>
                <Link href="/dashboard" className="flex items-center gap-2">Go to Dashboard <ArrowRight className="h-4 w-4" /></Link>
              </Button>
            ) : (
              <Button size="lg" className="backdrop-blur-sm bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg shadow-indigo-500/30 border-0 transition-all duration-300 hover:scale-105" asChild>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {features.map((feature, index) => (
            <Card key={index} className={`h-full backdrop-blur-md bg-background/40 border ${featureColors[index % featureColors.length].border} shadow-lg hover:shadow-xl transition-all duration-300 hover:translate-y-[-5px] overflow-hidden`}>
              <div className={`absolute inset-0 ${featureColors[index % featureColors.length].bg} opacity-30 pointer-events-none`} />
              <CardHeader className="relative z-10">
                <div className="flex items-center space-x-3">
                  <div className={`p-3 ${featureColors[index % featureColors.length].bg} rounded-xl backdrop-blur-sm border ${featureColors[index % featureColors.length].border}`}>
                    <div className={featureColors[index % featureColors.length].icon}>
                      {feature.icon}
                    </div>
                  </div>
        ...
        <Card className="backdrop-blur-md bg-background/40 border border-indigo-500/20 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
          ...
          <CardHeader className="relative z-10">
            <CardTitle className="text-2xl font-semibold text-indigo-950 dark:text-indigo-100">How It Works</CardTitle>
            <CardDescription className="text-indigo-700/70 dark:text-indigo-300/70">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-3 p-5 rounded-xl backdrop-blur-sm bg-background/30 border border-indigo-500/20 shadow-sm relative overflow-hidden group hover:border-indigo-500/40 transition-all duration-300">
                ...
                <div className="flex items-center gap-3 relative z-10">
                  <div className="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center text-indigo-600 dark:text-indigo-300 font-bold">1</div>
                  <h4 className="font-semibold text-lg text-indigo-950 dark:text-indigo-100">Create Interviews</h4>
                </div>
              ...
              <div className="space-y-3 p-5 rounded-xl backdrop-blur-sm bg-background/30 border border-purple-500/20 shadow-sm relative overflow-hidden group hover:border-purple-500/40 transition-all duration-300">
                ...
                <div className="flex items-center gap-3 relative z-10">
                  <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center text-purple-600 dark:text-purple-300 font-bold">2</div>
                  <h4 className="font-semibold text-lg text-purple-950 dark:text-purple-100">Conduct Assessments</h4>
                </div>
              ...
              <div className="space-y-3 p-5 rounded-xl backdrop-blur-sm bg-background/30 border border-violet-500/20 shadow-sm relative overflow-hidden group hover:border-violet-500/40 transition-all duration-300">
                ...
                <div className="flex items-center gap-3 relative z-10">
                  <div className="w-8 h-8 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-600 dark:text-violet-300 font-bold">3</div>
                  <h4 className="font-semibold text-lg text-violet-950 dark:text-violet-100">Evaluate Candidates</h4>
                </div>
